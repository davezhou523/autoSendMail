...
type (
	...
	SearchContact struct {
		Id            uint64    `db:"id"`
		IsSend        int64     `db:"is_send"`         // 是否发送邮件,1:发送，2：不发送
		...
		LastContentId uint64    `db:"last_content_id"` // 最近一次内容id
		Keyword       string    `db:"keyword"`         // 关键词
		Domain        string    `db:"domain"`          // 域名
		Lr            string    `db:"lr"`              // 语言
		Gl            string    `db:"gl"`              // 最终用户的地理位置
		...
		Company       string    `db:"company"` // 用户的客户公司
...
func (m *defaultSearchContactModel) Insert(ctx context.Context, data *SearchContact) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.IsSend, data.IsReturn, data.Email, data.Phone, data.Category, data.UserId, data.CompanyId, data.LastContentId, data.Keyword, data.Domain, data.Lr, data.Gl, data.Url, data.Location, data.Md5, data.FirstName, data.LastName, data.Title, data.Company, data.Industry, data.Note)
	return ret, err
}
...
func (m *defaultSearchContactModel) Update(ctx context.Context, data *SearchContact) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, data.IsSend, data.IsReturn, data.Email, data.Phone, data.Category, data.UserId, data.CompanyId, data.LastContentId, data.Keyword, data.Domain, data.Lr, data.Gl, data.Url, data.Location, data.Md5, data.FirstName, data.LastName, data.Title, data.Company, data.Industry, data.Note, data.Id)
	return err
}
